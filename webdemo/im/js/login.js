var Login = {
    ...
    /*   requestLogin: function (account, pwd) {
        $.ajax({
          type: 'POST',
          url: "http://127.0.0.1:8050/webApi/nimApi/login.html",//自己公司的login的网址
          data: JSON.stringify({
            username: account,
            password: pwd,
          }),
    ...
    requestLogin: function (account, pwd) {
        fetch('/nimApi/login.html', {
            ...
            // TODO: 根据后台返回的参数判断老师的身份, 跳转到 mainTeacher 页面
            var goPage = res.data.neteaseAccid == 2 ? './mainTecher.html' : './mainStu.html';
            window.location.href = goPage;
        });
        /*$.ajax({
            type: 'GET',
            //url: "http://127.0.0.1:8050/webApi/nimApi/login.html",//自己公司的login的网址
            url: "/bylogin",//自己公司的login的网址
            data: {
                username: account,
                password: pwd,
            },
